...
WITH test_data( vacancy_id,
    ...
    SELECT generate_series(1, 10000)                                AS vacancy_id,
           now() - ('1 day'::interval * (random()*364::integer))    AS created,
           random() * 500::integer                                  AS employer_id,
           md5(random()::text)                                      AS position_name,
           1 + random() * 99::integer                               AS area_id,
           1 + random() * 99::integer                               AS specialization_id,
           1 + random() * 99::integer                               AS industry_id,
           md5(random()::text)                                      AS vacancy_description,
           round((random() * (90000 - 10000) + 10000)::integer, -3) AS compensation_from,
           round((random() * (40000 - 5000) + 5000)::integer, -3)   AS compensation_interval,
           false
)
INSERT INTO vacancy(vacancy_id, created, employer_id, position_name, area_id, specialization_id, industry_id, vacancy_description, compensation_from, compensation_to, compensation_gross)
SELECT vacancy_id,
       created,
       employer_id,
       position_name,
...
with test_data( resume_id,
    ...
    SELECT generate_series(1, 100000)                                   AS resume_id,
           ...
           1 + random() * 99::integer                                   AS specialization_id,
           md5(random()::text)                                          AS position_name,
           round((random() * (120000 - 10000) + 10000)::integer, -3)    AS compensation,
...
SELECT v.vacancy_id,
       resume_id,
       r.created::date + (random()*(now()::date - r.created))::integer AS created
FROM vacancy v
